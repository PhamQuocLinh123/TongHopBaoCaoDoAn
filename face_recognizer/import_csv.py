...
class InsertData:
    def __init__(self,root):
        ...
        img_btn1 = img_btn1.resize((120, 30), PIL.Image.BILINEAR)
        self.photobtn1 = ImageTk.PhotoImage(img_btn1)


        title = Label(right_lbl, text="Thêm danh sách người dùng: ", font=("times new roman", 12, "bold"), bg="white",
        ...
        title1 = Label(right_lbl, text="Thêm danh sách ngày: ", font=("times new roman", 12, "bold"), bg="white",
    ...
    def insert_stu(self):
        try:
            global mydata
            mydata.clear()
            fln = filedialog.askopenfilename(initialdir=os.getcwd() + "/Import", title="Open CSV",
            ...
            conn.commit()
            messagebox.showinfo("Thông báo", "Thêm danh sách người dùng!!!")
            conn.close()
    ...
    def insert_less(self):
        try:
            ...
            for index, row in df.iterrows():
                mydata.append((row.iloc[0], row.iloc[1], row.iloc[2], str(row.iloc[3]).replace("'", ""), row.iloc[4]))
            ...
            my_cursor = conn.cursor()
            sql_insert_query = "insert into lesson values(%s,%s,%s,%s,%s)"
            my_cursor.executemany(sql_insert_query, mydata)
            conn.commit()
            messagebox.showinfo("Thông báo", "Thêm danh sách thành công!!!")
            conn.close()
        except Exception as es:
            messagebox.showerror("Error", f"Due To:{str(es)}", parent=self.root)
